...
namespace SAM.Analytical.Revit
{
    ...
    {
        public static List<Wall> TrimOrExtendWall(this IEnumerable<Wall> walls, double maxDistance)
        {
            ...
            foreach (KeyValuePair<double, Dictionary<Wall, Segment2D>> keyValuePair in dictionary)
            {
                ...
                List<Tuple<Wall, Segment2D, List<int>>> tupleList = new List<Tuple<Wall, Segment2D, List<int>>>();
                foreach(KeyValuePair<Wall, Segment2D> keyValuePair_Wall in keyValuePair.Value)
                {
                    ...
                    if (indexes.Count > 0)
                        tupleList.Add(new  Tuple<Wall, Segment2D, List<int>>(keyValuePair_Wall.Key, keyValuePair_Wall.Value, indexes));
                }

                

                List<Tuple<Wall, Segment2D>> tupleList_Result = new List<Tuple<Wall, Segment2D>>();
                foreach(Tuple<Wall, Segment2D, List<int>> tuple in tupleList)
                {
                ...
                foreach(Tuple<Wall, Segment2D> tuple in tupleList_Result)
                {
